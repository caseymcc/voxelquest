...
void pack16(float num, float &outR, float &outG)
{
    ...
    outR=float(ir)/255.0f;
    outG=float(ig)/255.0f;
...
{
    float intPart;
    return modf(sin(x * 12.9898f+y*78.233f) * 43758.5453f, &intPart);
}

int iSeedRand2(float x, float y, int minV, int maxV)
{
    float res=fSeedRand2(x, y);
    return minV+std::max(int((maxV+1-minV) * res), 0);
}
...
float FIVector4::operator[] (int ind)
{ //float&
...
void FIVector4::fixForRot()
{
    ...
    iv4.x=std::max(iv4.x, 0);
    iv4.y=std::max(iv4.y, 0);
    iv4.z=std::max(iv4.z, 0);
...
void FIVector4::addXYZ(float scalar, float multiplier)
{
...
void FIVector4::addXYZ(float scalarX, float scalarY, float scalarZ, float multiplier)
{
...
void FIVector4::addXYZW(float scalarX, float scalarY, float scalarZ, float scalarW, float multiplier)
{
    fv4.x+=scalarX*multiplier;
    fv4.y+=scalarY*multiplier;
    fv4.z+=scalarZ*multiplier;
...
void FIVector4::addXYZRef(FIVector4 *scalar, float multiplier)
{
...
void FIVector4::normalizeBounds(FIVector4 *minBounds, FIVector4 *maxBounds)
{
    float temp;
...
bool FIVector4::intersectInt(FIVector4 *aMin, FIVector4 *aMax, FIVector4 *bMin, FIVector4 *bMax)
{
...
bool FIVector4::intersect(FIVector4 *aMin, FIVector4 *aMax, FIVector4 *bMin, FIVector4 *bMax)
{
...
void FIVector4::growBoundary(FIVector4 *minB, FIVector4 *maxB, FIVector4 *minV, FIVector4 *maxV)
{
...
float FIVector4::wrapDistance(FIVector4 *otherVec, int maxPitch, bool doSet)
{
    ...
    for(i=-1; i<=1; i++)
    {
        for(j=-1; j<=1; j++)
        {
            ...
            otherVec->addXYZ((float)(i * maxPitch), (float)(j * maxPitch), 0.0f, 1.0f);
    ...
    if(doSet)
    {
        otherVec->addXYZ((float)(bestI * maxPitch), (float)(bestJ * maxPitch), 0.0f);
    }
...
void FIVector4::cross(FIVector4 *outVec, FIVector4 *v1, FIVector4 *v2)
{
...
{
    float intPart;
    return abs(modf(sin(xv*433.2f+yv*522.9f+zv*839.4f) * 43758.8563f, &intPart));
}
...
{
    float intPart=0.0f;
    int res=(int)(abs(modf(sin(seedPos->dot(seedVals)) * 43758.8563f, &intPart))*1000000.0f);
...
{
    int begInDeg=(int)(begInRad*180.0f/(float)M_PI);
    int endInDeg=(int)(endInRad*180.0f/(float)M_PI);

    float shortest_angle=(float)(((((endInDeg-begInDeg)%360)+540)%360)-180);

    return shortest_angle*amount * (float)M_PI/180.0f;
}
