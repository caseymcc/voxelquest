        ...
        for(j=0; j<holdersPerBlock; j++)
        {
            ...
            if(res!=0)
            {
                for(k=0; k<TOT_MAP_DIRS; k++)
                {
                    ...
                    if((res==res2)&&(touchesMap(i, j, E_CT_ROAD, 0)==0))
                    { // house id matches, there is a house in that direction
...
void GameBlock::applyWingValues(
...
float GameBlock::fIsNearTerrain(
