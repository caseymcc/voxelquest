...
int MyGLHelper::registerGraphicsShape(const float* vertices, int numvertices, const int* indices, int numIndices)
{
	int shapeId = m_data->m_glApp->m_renderer->registerShape(vertices, numvertices,indices,numIndices);
	return shapeId;
}

int MyGLHelper::registerGraphicsInstance(int shapeIndex, const float* position, const float* quaternion, const float* color, const float* scaling)
{
...
void MyGLHelper::syncPhysicsToGraphics(const btDiscreteDynamicsWorld* rbWorld)
{
...
void MyGLHelper::render(const btDiscreteDynamicsWorld* rbWorld)
{
...
void MyGLHelper::drawText3D( const char* txt, float posX, float posY, float posZ, float size)
{
